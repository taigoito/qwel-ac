...
Particle = (function() {
  function Particle(x, y, min1, max1, radius1, force1, hue1, img1) {
    this.min = min1;
    this.max = max1;
    this.radius = radius1;
    this.force = force1;
...
Sketch = (function() {
  ...
  Sketch.prototype.creation = function(num, hue) {
    var force, i, images, img, j, k, max, min, r, radius, ref, x, y;
    images = [];
    for (i = j = 0; j < 10; i = ++j) {
      images[i] = new Image;
    }
    images[0].src = this.homeUrl + 'images/scratch/logo.svg';
    images[1].src = this.homeUrl + 'images/scratch/cat.svg';
    images[2].src = this.homeUrl + 'images/scratch/tera.svg';
    images[3].src = this.homeUrl + 'images/scratch/gobo.svg';
    images[4].src = this.homeUrl + 'images/scratch/knight.svg';
    images[5].src = this.homeUrl + 'images/scratch/hippo.svg';
    images[6].src = this.homeUrl + 'images/scratch/wizard.svg';
    images[7].src = this.homeUrl + 'images/scratch/dinosaur.svg';
    images[8].src = this.homeUrl + 'images/scratch/soccerball.svg';
    images[9].src = this.homeUrl + 'images/scratch/spaceship.svg';
    for (i = k = 0, ref = num; 0 <= ref ? k < ref : k > ref; i = 0 <= ref ? ++k : --k) {
      ...
      img = images[i];
      this.particles[i] = new Particle(x, y, min, max, radius, force, hue, img);
    }
  };
  ...
  Sketch.prototype.draw = function() {
    ...
    for (i = j = 0, ref = len; 0 <= ref ? j < ref : j > ref; i = 0 <= ref ? ++j : --j) {
      ...
      ctx.drawImage(this.particles[i].img, this.particles[i].location.x - this.particles[i].radius / 2, this.particles[i].location.y - this.particles[i].radius / 2, this.particles[i].radius, this.particles[i].radius);
      ctx.beginPath();
      ...
      ellipseGradient.addColorStop(0, 'hsla(' + this.particles[i].hue + ', 85%, 100%, .5)');
